...
const LiftingStateUpDemo = () => {
  ...
  return (
    <div style={{backgroundColor: "purple"}}>
        ...
        <InputPersonComponent addPerson={addPerson} nextPerson={newPerson}/>
        </div>        
    </div>
  )
}
...
const AllPersonsComponent = ({persons,deletePerson, editPerson}) => {
    return (
        <div style={{width: "300px", backgroundColor: "brown"}}>
            ...
            <ul>
                {persons.map(person => {
                    return <li key={person.id}>{person.name} (id: {person.id}) (<FaPen style={{color:"orange", cursor: "pointer"}} onClick={() => editPerson(person.id)}/> / <FaTimes style={{color:"red", cursor: "pointer"}} onClick={() => deletePerson(person.id)}/>)</li>
                })}
            </ul>
        </div>
    )
